...
class Mage_ProductAlert_AddController extends Mage_Core_Controller_Front_Action {
    ...
    public function stockAction() {
        ...
        try {
            ...
            $clienteNome = $cliente->getName();
            $clienteEmail = $cliente->getEmail();
            
            $produtoNome = $product->getName();
            $produtoLink = $product->getProductUrl();
            
            $this->sendOwnerNotification($clienteNome, $clienteEmail, $produtoNome, $produtoLink);
        } catch (Exception $e) {
            $session->addException($e, $this->__('Unable to update the alert subscription.'));
        }
        $this->_redirectReferer();
    ...
    public function sendOwnerNotification($clienteNome, $clienteEmail, $produtoNome, $produtoLink) {

        $emailTemplate = Mage::getModel('core/email_template')
                ->loadByCode('alerta_cliente_fila_espera');

        $emailTemplateVariables = array();
        $emailTemplateVariables['cliente_nome'] = $clienteNome;
        $emailTemplateVariables['cliente_email'] = $clienteEmail;
        $emailTemplateVariables['produto_nome'] = $produtoNome;
        $emailTemplateVariables['produto_link'] = $produtoLink;
        ...
        try {
            $emailTemplate->send($email, 'Galpón Rural', $emailTemplateVariables);
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    }
